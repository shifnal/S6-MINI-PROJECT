...
def login_page():
    st.markdown(
        ...
        <style>
            .stApp {{
                background-image: url('data:image/jpg;base64,{get_image_base64('login_background.jpg')}');
...
def get_disease_info(disease_name):
    try:
        ...
        response = supabase.table("disease")\
            .select("disease_name, description, treatment_suggestions(treatment_description), fertiliser(fertiliser_name, type)")\
        ...
        if response.data:
            ...
            return {
                ...
                "Treatment": disease_info["treatment_suggestions"][0]["treatment_description"] if disease_info["treatment_suggestions"] else "No specific treatment available",
                "Fertiliser": f"{disease_info['fertiliser'][0]['fertiliser_name']} ({disease_info['fertiliser'][0]['type']})" if disease_info["fertiliser"] else "No recommended fertiliser"
            }
        else:
            return None
    except Exception as e:
